...
アプリケーションをご利用いただくにあたり、以下の利用規約に同意していただく必要があります。

### 1. 利用について
*   本アプリケーションはMITライセンスの下で提供されています。ただし、一部のコンポーネントには異なるライセンスが適用されています。商用・非商用を問わず、自由に使用、複製、配布、変更が可能ですが、異なるライセンスが適用されているコンポーネントに関しては、そのライセンス条件に従ってご利用ください。
*   悪用を目的とした利用や、本アプリケーションの不正改変は禁止されています。

### 2. VRChatの規約遵守
*   本アプリケーションを使用する際は、VRChatの規約に従ってください。VRChatの規約に違反する行為は禁止されています。

### 3. 免責事項
*   本アプリケーションの利用により発生したいかなるトラブル、損害、データの損失についても、製作者は一切の責任を負いません。全ての利用は利用者の自己責任で行ってください。

### 4. 規約の変更
*   本利用規約は、事前の通知なしに変更されることがあります。変更後の規約は、新しいバージョンのリリースとともに適用されます。

### 5. その他
*   本アプリケーションの利用にあたって、法令を遵守してください。
*   本アプリケーションの提供は、現状のまま行われ、いかなる保証もありません。不具合やバグに対するサポート、アップデートの提供についても保証されません。
